...
import { resolveToolIcon } from "@/lib/gemini";
...
export default function Home() {
  ...
  useEffect(() => {
    if (r?.tools && r.tools.length > 0) {
      setActiveTools(r.tools.map((t: { name: string; icon?: string }) => ({
        name: t.name,
        icon: t.icon && !t.icon.includes("/tools/image") ? t.icon : resolveToolIcon(t.name),
      })));
    }
  }, [r?.tools]);
                                  ...
                                  >
                                    <div className="relative w-4 h-4 flex items-center justify-center shrink-0">
                                      {tool.icon ? (
                                        ...
                                        <span
                                          className={`absolute inset-0 w-4 h-4 rounded flex items-center justify-center text-[7px] font-bold uppercase ${
                                            isSelected
                                              ? "bg-black/[0.08] dark:bg-white/[0.1] text-[#1A1A1A] dark:text-[#F0EDE7]"
                                              : "bg-black/[0.06] dark:bg-white/[0.06] text-[#7A736C] dark:text-[#9E9893]"
                                          }`}
                ...
                {activeTools.map((tool, i) => (
                  ...
                  >
                    ...
                    {tool.icon ? (
                      ...
                      <div className="w-full h-full rounded-md bg-black/[0.06] dark:bg-white/[0.08] flex items-center justify-center text-[11px] font-bold text-[#7A736C] dark:text-[#9E9893] uppercase">
                        {tool.name.slice(0, 2)}
                      </div>
                    )}
                  </motion.div>
